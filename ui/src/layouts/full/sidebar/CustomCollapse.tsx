...
const CustomCollapse: React.FC<{
  ...
  return (
    <div className={twMerge('transition-all duration-300')}>
      <div
        className={twMerge(
          'flex cursor-pointer mb-1 items-center justify-between rounded-lg px-4 py-[11px] gap-3 text-[15px] leading-[normal] font-normal text-link nav-cover hover:text-primary   dark:hover:text-primary',
          className,
        )}
        onClick={onClick}
      >
        ...
        <div className="flex items-center gap-0.5">
          {isPro ? (
            <span className="py-1 px-2.5 text-[10px] bg-secondary text-white rounded-full">
              Pro
            </span>
          ) : null}
          <HiOutlineChevronDown
